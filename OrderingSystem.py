...
class Order:
    ...
    def __init__(self):
        hot_coffee = [ "Cafe Latte","Cafe Mocha","Cappuccino","Americano","Steamed Milk","Espresso","Hazelnut","House Decaf","Espresso Decaf"]
        cold_coffee = [ "Iced Latte","Iced Cappuccino","Iced Flat White","Iced Americano Black","Iced Mocha","Iced Cortado","Cold Brew with whipped milk","Strawberry Mint Iced Tea","Strawberry Lemonade","Lemon Ginger Iced Tea" ]
        milk_tea = ["Almond Milk Tea","Chocolate Milk Tea","Fruit Magic Tea","Coffee Milk Tea","Ginger Milk Tea","Hawaiian Fruit Tea","Lavender Milk Tea","Taro Milk Tea","Jack Fruit Tea","Honey Lemon Tea" ]
        
        shop_products = {"Hot Coffee" : hot_coffee, "Cold Coffee" : cold_coffee, "Milk Tea" : milk_tea}
    ...
    def print_shop_menu(self):
        ...
        print("-"*60)
        print ('\033[1m' , "Menu\t\t\t\t", Size.SMALL.value, "- P" , Price.SMALL.value, "|" , Size.LARGE.value, "- P" ,Price.LARGE.value, '\033[0m')
        order = Order()
    ...
    def hot_coffee_order(self):
        print("-"*60)
        try: self.choice = int(input("Your order is: "))
        except ValueError:
            print ("Invalid input!")
        print("-"*60)
        self.order_product = self.shop_products["Hot Coffee"][self.choice-1]
        print("\nYou selected", self.shop_products["Hot Coffee"][self.choice-1])
        self.to_pay = 0
    ...
    def cold_coffee_order(self):
        ...
        print("\nYou selected", self.shop_products["Cold Coffee"][self.choice-1])
        self.to_pay = 0
        self.coffee_choice = 0
        self.coffee_choice = int(input("What size do you want? [1] Small or [2] Large -> "))
    ...
    def print_orders(self):
        ...
        for x in range(len(bev_order)):
            print ("{:30}{:10}{:15}".format(bev_order[x],bev_quantity[x],bev_price[x]))
    ...
    def write_to_file(self):
        ...
        file.write("-"*60 + '\n')
        file.write("{:33}{:20}{:10}".format("Beverage order/s","Qty","Price"))
        file.write('\n'+"-"*60)
        for x in range(len(bev_order)):
            file.write("\n{:35}{:20}{:15}".format(bev_order[x],str(bev_quantity[x]),str(bev_price[x])))
        file.write("\n" + "-"*60)
